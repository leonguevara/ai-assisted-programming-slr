...
Salida:
    ...
    ./slr_outputs/figures/*.png  # pie charts, stacked bars y heatmaps
...
def save_pie_chart(
    ...
    """
    Guarda una gráfica de pie/donut para distribuciones parte-todo.

    - Muestra conteo + porcentaje en cada rebanada suficientemente grande.
    - Deja fuera etiquetas internas en rebanadas muy pequeñas para evitar saturación.
    - Mantiene la leyenda con todas las categorías.
    """
    plot_data = data[[label_col, value_col]].dropna().copy()
    plot_data = plot_data[plot_data[value_col] > 0]
    total = plot_data[value_col].sum()
    ...
    wedges, texts, autotexts = ax.pie(
        plot_data[value_col],
        labels=None,
        autopct=autopct(plot_data[value_col]),
        startangle=90,
        counterclock=False,
        pctdistance=0.72,
        colors=colors,
    ...
    legend_labels = [
        f"{row[label_col]} — {int(row[value_col])} ({row[value_col] / total * 100:.2f}%)"
...
save_pie_chart(
    ...
    "Distribution by primary dimension", "01_dimension_distribution_pie.png"
)

save_pie_chart(
    tables["segment_distribution"], "Segment", "Papers",
    "Epistemological distribution by segment", "02_segment_distribution_pie.png"
)

save_pie_chart(
    tables["quality_distribution"], "Evidence quality", "Papers",
    "Distribution by evidence quality", "03_quality_distribution_pie.png"
)

save_pie_chart(
    tables["year_distribution"], "Publication year", "Papers",
    "Temporal distribution of the corpus", "04_year_distribution_pie.png"
)
...
save_pie_chart(
    ...
    "Dimension", "Papers",
    "Core dataset distribution by dimension", "10_core_dimension_distribution_pie.png"
)
...
if "Item Type" in df.columns:
    ...
    save_pie_chart(
        ...
        "Distribution by item type", "18_item_type_distribution_pie.png"
    )
...
if "Nivel_Soporte" in df.columns:
    ...
    save_pie_chart(
        ...
        "Distribution by support level", "19_support_level_distribution_pie.png"
    )
...
if "Source" in df.columns:
    ...
    save_pie_chart(
        ...
        "Distribution by source", "20_source_distribution_pie.png"
    )
